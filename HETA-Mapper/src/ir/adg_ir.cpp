...
ADG* ADGIR::parseADG(json& adgJson, bool isSub){
    ...
    if(!isSub){
        ...
        adg->GPEgroupResize(adgJson["num_colum"].get<int>());
        adg->GIBgroupResize(adgJson["num_row"].get<int>() + 1);
        adg->LSUgroupResize(2);
    }
    //adg->setMaxII(4);
...
ADGNode* ADGIR::parseADGNode(json& nodeJson){
    ...
    if(type == "GPE" || type == "GIB" || type == "IB" || type == "OB"|| type == "LSU"){
        ...
        //@Yuan TODO: set the maximum configmem size accroding to json file
