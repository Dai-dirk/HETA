...
//@yuan: for HETA DSE
void ADG::setGPEgroup(int gpeIdx, ADGNode* node){
    assert(_GPEgroup.size() > gpeIdx);
    _GPEgroup[gpeIdx].emplace(node);
}

void ADG::setGIBgroup(int gibIdx, ADGNode* node){
    assert(_GIBgroup.size() > gibIdx);
    _GIBgroup[gibIdx].emplace(node);
}